...
//char ** -> permite criar um array de char *
//char * - > permite criar um array de char

char **gerar_tabuleiro(size_t dim)
{
    int i,j;
    char **tabuleiro = (char **)malloc(dim * sizeof(char*)); // Aloca um array de *char

    for(i = 0; i < dim; i++) // percorre o array de *char
    {
        tabuleiro[i] = (char*)malloc(dim * sizeof(char)); // Aloca um array de char
        for(j = 0; j < dim; j++) // Pecorre o array de char
        {
            tabuleiro[i][j] = ' '; // Atribui ' ' ao char
        }
    }
